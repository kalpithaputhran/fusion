...
class Management extends Main
{
	...
	public function __construct()
	{
		...
		$this->id = $_SESSION['management'];
	}
		parent::__construct();
	}

	public function loginManagement($name,$password)
	{
		try{
			$stmnt=$this->conn->prepare("select m_id from management where m_username=:m_username AND m_password=:m_password");
			$stmnt->bindParam("m_username", $name,PDO::PARAM_STR) ;
			$stmnt->bindParam("m_password", $password,PDO::PARAM_STR) ;
			$stmnt->execute();
			$count=$stmnt->rowCount();
			$res=$stmnt->fetch(PDO::FETCH_ASSOC);
			$id = $res['m_id'];
			if($count){
				$_SESSION['management']= $id;
				return true;
			}else{
				return false;
			}
...
public function cud($res,$message){
	...
	public function get_payroll_det(){
		try {
			$stmt = $this->conn->prepare("SELECT * FROM payroll");
			$stmt->execute();
			return $stmt;	
		} catch (PDOException $e) {
			echo $e->getMessage();
...
public function get_course(){
	...
	public function addStudent($registerno,$dateofregistration,$studentphoto,$studentname,$dob,$gender,$address,$contactno1,$contactno2,$email,$parentname,$parentcontact,$collegename,$placeofcollege,$previousqualification,$overallpercentage,$noofbacklogs,$cid,$courseamt,$paidamt){
		try {
			$stmt = $this->conn->prepare("INSERT INTO student(registerno,dateofregistration,studentphoto,studentname,dob,gender,address,contactno1,contactno2,email,parentname,parentcontact,collegename,placeofcollege,previousqualification,overallpercentage,noofbacklogs,cid) VALUES(:registerno,:dateofregistration,:studentphoto,:studentname,:dob,:gender,:address,:contactno1,:contactno2,:email,:parentname,:parentcontact,:collegename,:placeofcollege,:previousqualification,:overallpercentage,:noofbacklogs,:cid)");
...
public function getstaff(){
		try {
			$stmt = $this->conn->prepare("SELECT * FROM staff where staffstatus='Active'");
			$stmt->execute();
			return $stmt;	
		} catch (PDOException $e) {
			echo $e->getMessage();
...
public function getStaffsalary(){
		  try {
		   $stmt = $this->conn->prepare("SELECT salary FROM staff WHERE staff_id = '$staff_id'");
		   $stmt->execute();
		   return $stmt;
		} catch (PDOException $e) {
		   echo $e->getMessage();
